...
http.onreadystatechange = function() {
  ...
  document.body.style.backgroundImage = 'url("' + images[data.list[0].weather[0].main].background; + '")';
  ...
  details.forEach(e => {
    document.getElementById(e).innerHTML =
      e === 'focus-type' ? data.list[0].weather[0].main:
      //multiplication and division to achieve 2 decimal places
      e === 'focus-temp' ? Math.round(data.list[0].main.temp * 10 ) / 10 + " o".sup() + "C":
      e === 'location' ? data.city.name + ', ' + data.city.country:
      '<i class="' + images[data.list[0].weather[0].main].icon + ' fa-10x focus-img"></i>';
  })
    ...
    childArray.map(m => m.id)
              ...
              .forEach(x => {
                ...
                //index*8 used to get one regular daily reading. API provides 3 hourly (24/3 = 8)
                  id === 'type' ? data.list[i*8].weather[0].main:
                  id === 'pic' ? '<i class="' + images[data.list[i*8].weather[0].main].icon + ' fa-3x"></i>':
                  id === 'date' ? date.toLocaleString('en-gb').slice(0, 5):
                  //multiplication and division to achieve 2 decimal places
                  Math.round(data.list[i*8].main.temp * 10 ) / 10 + " o".sup() + "C";
              });
  });  
}